...
module.exports = {
  ...
  async run(client, message, args) {
    ...
    if (!args.length)
      return message
        .reply(`v.playlist ▶️ YouTube Playlist URL | 📀 Playlist Name`)
        .catch(console.error);
    if (!channel) return message.channel.send("You need to join a voice channel first!").catch(console.error);

    const permissions = channel.permissionsFor(message.client.user);
    if (!permissions.has("CONNECT"))
      return message.channel.send(":x: Cannot connect to voice channel, missing permissions");
    ...
    videos.forEach((video) => {
      ...
      if (serverQueue) {
        ...
        if (!PRUNING)
          message.channel
            .send({embed: { description: `✅ **${song.title}** has been added to the queue by ${message.author}`, color: "GREEN"}})
            .catch(console.error);
      } else {
